...
void CG_Player(centity_t *cent) {
	...
	VectorCopy(cent->lerpOrigin, legs.lightingOrigin);
        ...
        if (cent->currentState.number == cg.snap->ps.clientNum) {
                torso.radius = 0.7; // Mix3r: scale runes down for third person
                if (cg.renderingThirdPerson && !cg.zoomed) {
                       // Mix3r smooth out stair climbing for local third person player
		          ...
		          legs.lightingOrigin[2] = legs.origin[2] -= cg.stepChange * (STEP_TIME - head.origin[2]) / STEP_TIME;
                       ...
                       // Mix3r setup offset and draw first person legs
                       ...
                       // let's bend fake first person legs legs back a bit
	...
	CG_PlayerPowerups(cent, &torso, &head.origin[2]);
        ...
        if ( cgs.clientinfo[ cent->currentState.clientNum ].eyepos[1] == 0) {
                ...
                // Mix3r_Durachok: naykaH arachnotron gun position hack
