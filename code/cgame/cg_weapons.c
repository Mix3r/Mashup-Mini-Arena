...
//#endif
	...
	case WP_MACHINEGUN:
		...
		weaponInfo->flashSound[2] = trap_S_RegisterSound( va("sound/weapons/%s/machgf3b.wav",cg.mg_subst), qfalse );
		weaponInfo->flashSound[3] = trap_S_RegisterSound( va("sound/weapons/%s/machgf4b.wav",cg.mg_subst), qfalse );
		weaponInfo->ejectBrassFunc = CG_MachineGunEjectBrass;
		cgs.media.bulletExplosionShader = trap_R_RegisterShader( "bulletExplosion" );
		weaponInfo->lfx = 62;
		break;
...
void CG_RegisterItemVisuals( int itemNum )
{
	...
	item = &bg_itemlist[ itemNum ];
        ...
        if (itemNum > 0 && itemNum <= 60) {
                item->pickup_name = COM_Localize(itemNum);
        }
...
// LEILEI ENHANCEMENT
	...
	} else if (weapon != WP_NAILGUN) CG_Bleed( origin, entityNum );
