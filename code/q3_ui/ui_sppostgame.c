...
// Mix3r_Durachok: load respective language version of sound or fallback to ru_ folder prefix
sfxHandle_t UI_CacheLocalizedSound(char *snd) {
        ...
        rethandle = trap_S_RegisterSound(snd, qfalse);
        if (rethandle) {
                return rethandle;
        }
        trysnd = va("ru_%s",snd);
        return trap_S_RegisterSound(trysnd, qfalse);
}
